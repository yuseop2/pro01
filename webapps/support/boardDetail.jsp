...
<%
	...
	try {
		...
		try {
			...
			sql = "select board.w_num as w_num, board.title as title, board.content as content, member.name as name, board.w_date as w_date, board.author as author from board, member where board.author=member.id and board.w_num=?";
			try {
...
%>
				<tr>
					<td colspan="2">
						<a href="<%=path %>/support/support.jsp#page1" class="btn btn-primary">글 목록</a> &nbsp; &nbsp; &nbsp;
<%
					if(pid.equals(compId) || pid.equals("admin")){
%>
						<a href="<%=path %>/support/boardUpdate.jsp?w_num=<%=w_num %>" class="btn btn-cancle">글 수정</a>								
						<a href="<%=path %>/support/boardDel.jsp?w_num=<%=w_num %>" class="btn btn-primary">삭제</a>
<%
					}
%>							
					</td>
